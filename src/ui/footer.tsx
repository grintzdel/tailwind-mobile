...
import { ACTIVE_NAV_ITEM, GLASS_EFFECT, GRADIENT_BG } from '@/modules/shared/constants/style.constant'
...
const FooterLink: React.FC<FooterLinkProps> = ({ icon: Icon, href, isActive }: FooterLinkProps): React.JSX.Element => {
  return (
    <li>
      <Link href={href} className="relative block cursor-pointer rounded-full p-5">
        <div
  ...
  return (
    <nav>
      <ul className={`relative rounded-full ${GLASS_EFFECT.container} p-1`}>
        <div className={GLASS_EFFECT.border} style={GLASS_EFFECT.borderStyle} />
        <div className="relative z-10 flex flex-row items-center gap-1">{children}</div>
      </ul>
    </nav>
  )
}
...
export const Footer: React.FC = (): React.JSX.Element => {
  ...
  return (
    <footer className="absolute bottom-5 flex w-full justify-center">
      <FooterLinksList>
        <FooterLink icon={TiHomeOutline} href="/" isActive={mounted && pathname === '/'} />
        <FooterLink icon={CiMusicNote1} href="/library" isActive={mounted && pathname === '/library'} />
        <FooterLink icon={CiSettings} href="/settings" isActive={mounted && pathname === '/settings'} />
      </FooterLinksList>
    </footer>
  )
}
